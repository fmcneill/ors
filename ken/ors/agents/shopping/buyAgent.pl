...
buyAgent :-
	reconsult(buyAgentThy),
	reconsult(buyAgentSig),
%  reconsult('agents/shopping/buyAgentThy'),
%  reconsult('agents/shopping/buyAgentSig'),
	listen.

listen :-
	in(query(QAgent,buyAgent,QueryType,Query)),!,
	write('buyAgent has received a query ..'),nl,
	resolve(QueryType,Query,QAgent),
	listen.



% buyAgent, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(buyAgent,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(buyAgent,QAgent,problem)).
...
% this is what buyAgent actually does:
...
checkPreconds([H|T],AskList,WaitList,Agent,ThingsToAsk,WaitingThings) :-
	write('this is the current precond: '),write(H),nl,
	test,
	...
	;
		...
		;
		   ( member(H,PredList) ->
		    H,! 
		    ;	
		       (   write('need to check that '),write(H),write(' is a fact'),
			   fact(H),!
			;   
...
checkWithAgent([H|T],Agent) :-
	out(query(buyAgent,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,buyAgent,question,Answer)),
...
% this is to connect buyAgent to the server:
...
:- startServer(_),initialiseClient(_),buyAgent.
