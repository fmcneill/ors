...
joinGroupAgentOne :-
	reconsult(joinGroupAgentOneThy),
	reconsult(joinGroupAgentOneSig),
	listen.

listen :-
	in(query(QAgent,joinGroupAgentOne,QueryType,Query)),!,
	write('joinGroupAgentOne has received a query ..'),nl,
...
% joinGroupAgentOne, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(joinGroupAgentOne,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(joinGroupAgentOne,QAgent,problem)).
...
% this is what joinGroupAgentOne actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(joinGroupAgentOne,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,joinGroupAgentOne,question,Answer)),
...
% this is to connect joinGroupAgentOne to the server:
...
:- startServer(_),initialiseClient(_), joinGroupAgentOne.
