...
joinGroupAgentTwo :-
	reconsult(joinGroupAgentTwoThy),
	reconsult(joinGroupAgentTwoSig),
	listen.

listen :-
	in(query(QAgent,joinGroupAgentTwo,QueryType,Query)),!,
	write('joinGroupAgentTwo has received a query ..'),nl,
...
% joinGroupAgentTwo, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(joinGroupAgentTwo,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(joinGroupAgentTwo,QAgent,problem)).
...
% this is what joinGroupAgentTwo actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(joinGroupAgentTwo,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,joinGroupAgentTwo,question,Answer)),
...
% this is to connect joinGroupAgentTwo to the server:
...
:- startServer(_),initialiseClient(_), joinGroupAgentTwo.
