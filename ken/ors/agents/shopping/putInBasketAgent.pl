...
putInBasketAgent :-
	reconsult(putInBasketAgentThy),
	reconsult(putInBasketAgentSig),
%  reconsult('agents/shopping/putInBasketAgentThy'),
% reconsult('agents/shopping/putInBasketAgentSig'),
	listen.

listen :-
	in(query(QAgent,putInBasketAgent,QueryType,Query)),!,
	write('putInBasketAgent has received a query ..'),nl,
	resolve(QueryType,Query,QAgent),
	listen.



% putInBasketAgent, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(putInBasketAgent,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(putInBasketAgent,QAgent,problem)).
...
% this is what putInBasketAgent actually does:
...
checkPreconds([H|T],AskList,WaitList,Agent,ThingsToAsk,WaitingThings) :-
	(   findPredInfo(H,HName,HArity),
	    createList(HArity,List),
	    UnInstH =.. [HName|List],
	    member(UnInstH,AskList),!,
	    write(H),write(' needs to be checked'),nl,
	    % first, is this something i should ask about?  if so, store
	    checkPreconds(T,AskList,WaitList,Agent,[H|ThingsToAsk],WaitingThings)
    
	;
	    (	write(H),write(' doesnt need to be checked'),nl,
		member(H,WaitList),!,
...
checkWithAgent([H|T],Agent) :-
	out(query(putInBasketAgent,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,putInBasketAgent,question,Answer)),
...
% this is to connect putInBasketAgent to the server:
...
:- startServer(_),initialiseClient(_),putInBasketAgent.
