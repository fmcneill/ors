...
startServer(PId):-
        file_exists('alan.addr'),    % if file "alan.addr" (handle) exists
        see('alan.addr'),            % open it,
        read(_:_-PId),                 % get PId,
        seen,                          % close it, and
        up(PId),                       % (1) check that process is up
        !.
startServer(PId):-                         % otherwise...
        createServerProgram,               % create server program "alan.pl"
        startServerAux,                    % start it up
        sleep(2),                          % wait a bit... (to update file)
        see('alan.addr'),                % open file with Linda handle
        read(_:_-PId),                     % get PId
        seen.                              % close it
...
createServerProgram:-
    ...
    write('   pid(PId),'),nl,
    write('   linda((Host:Port)-(tell(\'alan.addr\'),'),nl,
    write('                      '),
...
startServerAux:-
    exec('echo "[\'alan.pl\']." | sicstus > /dev/null &',
         [null,null,null],
          _).
