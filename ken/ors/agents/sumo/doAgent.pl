...
doAAgent :-
	reconsult(doAAgentThy),
	reconsult(doAAgentSig),
	listen.

listen :-
	in(query(QAgent,doAAgent,QueryType,Query)),!,
	write('doAAgent has received a query ..'),nl,
...
% doAAgent, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(doAAgent,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(doAAgent,QAgent,problem)).
...
% this is what doAAgent actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(doAAgent,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,doAAgent,question,Answer)),
...
% this is to connect doAAgent to the server:
...
:- startServer(_),initialiseClient(_), doAAgent.
