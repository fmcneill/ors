...
tellAgent :-
	consult(tellAgentThy),
	consult(tellAgentSig),
	listen.

listen :-
	in(query(QAgent,tellAgent,QueryType,Query)),!,
	write('tellAgent has received a query ..'),nl,
...
% tellAgent, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(tellAgent,QAgent,Answer)),
	write('ive told '),write(QAgent),write(' '),write(Answer),nl.

resolve(_,_,QAgent) :-
	out(reply(tellAgent,QAgent,problem)),
	write('ive told '),write(QAgent),write(' '),write(problem),nl.
...
% this is what tellAgent actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(tellAgent,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,tellAgent,question,Answer)),
...
% this is to connect tellAgent to the server:
...
%%%%%%%%%%%%%%%%%%%%%% START SERVER AND CLIENT
 ...
 :- startServer(_),initialiseClient(_), tellAgent.
