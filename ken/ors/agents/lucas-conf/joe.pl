...
listen :-
	in(query(QAgent,joe,QueryType,Query)),!,
	write('joe has received a query ..'),nl,
...
% joe, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(joe,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(joe,QAgent,problem)).
...
% this is what joe actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(joe,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,joe,question,Answer)),
	Answer = H,
...
% this is to connect joe to the server:
...
:- startServer(_),initialiseClient(_), joe.
