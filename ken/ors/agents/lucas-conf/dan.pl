...
listen :-
	in(query(QAgent,dan,QueryType,Query)),!,
	write('dan has received a query ..'),nl,
...
% dan, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(dan,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(dan,QAgent,problem)).
...
% this is what dan actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(dan,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,dan,question,Answer)),
...
% this is to connect dan to the server:
...
:- startServer(_),initialiseClient(_), dan.
