...
listen :-
	in(query(QAgent,graham,QueryType,Query)),!,
	write('graham has received a query ..'),nl,
...
% graham, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(graham,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(graham,QAgent,problem)).
...
% this is what graham actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(graham,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,graham,question,Answer)),
...
% this is to connect graham to the server:
...
:- startServer(_),initialiseClient(_), graham.
