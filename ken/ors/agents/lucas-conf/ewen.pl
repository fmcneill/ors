...
listen :-
	in(query(QAgent,ewen,QueryType,Query)),!,
	write('ewen has received a query ..'),nl,
...
% ewen, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(ewen,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(ewen,QAgent,problem)).
...
% this is what ewen actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(ewen,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,ewen,question,Answer)),
...
% this is to connect ewen to the server:
...
:- startServer(_),initialiseClient(_), ewen.
