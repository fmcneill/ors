...
listen :-
	in(query(QAgent,colin,QueryType,Query)),!,
	write('colin has received a query ..'),nl,
...
% colin, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(colin,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(colin,QAgent,problem)).
...
% this is what colin actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(colin,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,colin,question,Answer)),
	Answer = H,
...
checkWaitingThings([H|T]) :-
	( H = calculation(Sum) ->
	    Sum,!
	;
	    write('not sure why i have to wait for '),write(H),nl
	),
...
% this is to connect colin to the server:
...
:- startServer(_),initialiseClient(_), colin.
