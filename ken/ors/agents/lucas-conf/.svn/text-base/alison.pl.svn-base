...
listen :-
	in(query(QAgent,alison,QueryType,Query)),!,
	write('alison has received a query ..'),nl,
...
% alison, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(alison,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(alison,QAgent,problem)).
...
% this is what alison actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(alison,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,alison,question,Answer)),
...
% this is to connect alison to the server:
...
:- startServer(_),initialiseClient(_), alison.
