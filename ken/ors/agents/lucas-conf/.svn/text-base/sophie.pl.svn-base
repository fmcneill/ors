...
listen :-
	in(query(QAgent,sophie,QueryType,Query)),!,
	write('sophie has received a query ..'),nl,
...
% sophie, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(sophie,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(sophie,QAgent,problem)).
...
% this is what sophie actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(sophie,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,sophie,question,Answer)),
...
% this is to connect sophie to the server:
...
:- startServer(_),initialiseClient(_), sophie.
