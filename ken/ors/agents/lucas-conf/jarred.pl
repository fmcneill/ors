...
listen :-
	in(query(QAgent,jarred,QueryType,Query)),!,
	write('jarred has received a query ..'),nl,
...
% jarred, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(jarred,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(jarred,QAgent,problem)).
...
% this is what jarred actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(jarred,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,jarred,question,Answer)),
...
% this is to connect jarred to the server:
...
:- startServer(_),initialiseClient(_), jarred.
