...
postUnitedKingdomAgent :-
	reconsult(postUnitedKingdomAgentThy),
	reconsult(postUnitedKingdomAgentSig),
	listen.

listen :-
	in(query(QAgent,postUnitedKingdomAgent,QueryType,Query)),!,
	write('postUnitedKingdomAgent has received a query ..'),nl,
...
% postUnitedKingdomAgent, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(postUnitedKingdomAgent,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(postUnitedKingdomAgent,QAgent,problem)).
...
% this is what postUnitedKingdomAgent actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(postUnitedKingdomAgent,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,postUnitedKingdomAgent,question,Answer)),
...
% this is to connect postUnitedKingdomAgent to the server:
...
:- startServer(_),initialiseClient(_), postUnitedKingdomAgent.
