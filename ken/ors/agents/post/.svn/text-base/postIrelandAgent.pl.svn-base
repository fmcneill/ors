...
postIrelandAgent :-
	reconsult(postIrelandAgentThy),
	reconsult(postIrelandAgentSig),
	listen.

listen :-
	in(query(QAgent,postIrelandAgent,QueryType,Query)),!,
	write('postIrelandAgent has received a query ..'),nl,
...
% postIrelandAgent, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(postIrelandAgent,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(postIrelandAgent,QAgent,problem)).
...
% this is what postIrelandAgent actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(postIrelandAgent,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,postIrelandAgent,question,Answer)),
...
% this is to connect postIrelandAgent to the server:
...
:- startServer(_),initialiseClient(_),postIrelandAgent.
