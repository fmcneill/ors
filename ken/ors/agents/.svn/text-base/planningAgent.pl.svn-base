...
plan :-
	...
	consult('translation'),
	nl,nl,nl,write('consulting the planner ...'),nl,nl,
	consult(planner),
	nl,nl,nl,write('consulting the ontology updater ...'),nl,nl,
	consult('update'),
    % the following files should not be consulted by the pa and are only the business of ors.  these will be altered shortly.
        nl,nl,nl,write('consulting the plan deconstructor ...'),nl,nl,
   	consult('planDecon'),
	nl,nl,nl,write('consulting the diagnostic algorithm ...'),nl,nl,
	consult('diagnosticAlgorithm'),
	nl,nl,nl,write('consulting the refinement system ...'),nl,nl,
	consult('refinement'),
	consult('metaRefinement'),
	%%%%% start ESC code %%%%%
	nl,nl,nl,write('consulting the experience sharing component ...'),nl,nl,
	consult('expClient'),
	%%%%% end ESC code %%%%%
	...
	now(EndTime),
	nl,nl,
	write('Started at '),write(StartTime),nl,
	write('Finished at '),write(EndTime),nl,nl.
	%%%%% end ESC code %%%%%.
...
defaultGoal('sh',has(shoppingAgent,ourMutualFriend)).
defaultGoal('butter', has(ourAgent,butter)).
defaultGoal('post','sent(fourman,thisLetter)').
...
processPlan([],Asserted,_Retracted,_,Just,RefinementList) :-
	shareExps(RefinementList), %%%%% ESC code %%%%%
...
waitForAnswer(SolvingAgent,Outcome,ExpectedQuestions,CurrentQuery,CurrentSurprisingQuestions,QueryList,LatestSurprisingQuestions,ok) :-
	...
	write(SolvingAgent),write(' asked me '),write(Ques),nl,
	solve(Type,Ques,Answer,SolvingAgent),
	write('I told '),write(SolvingAgent),write(' '),write(Answer),nl,
