...
addToBagAgent :-
	reconsult(addToBagAgentThy),
	reconsult(addToBagAgentSig),
	listen.

listen :-
	in(query(QAgent,addToBagAgent,QueryType,Query)),!,
	write('addToBagAgent has received a query ..'),nl,
...
% addToBagAgent, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(addToBagAgent,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(addToBagAgent,QAgent,problem)).
...
% this is what addToBagAgent actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(addToBagAgent,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,addToBagAgent,question,Answer)),
...
% this is to connect addToBagAgent to the server:
...
:- startServer(_),initialiseClient(_), addToBagAgent.
