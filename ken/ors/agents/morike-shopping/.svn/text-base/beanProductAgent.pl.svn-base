...
beanProductAgent :-
	reconsult(beanProductAgentThy),
	reconsult(beanProductAgentSig),
	listen.

listen :-
	in(query(QAgent,beanProductAgent,QueryType,Query)),!,
	write('beanProductAgent has received a query ..'),nl,
...
% beanProductAgent, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(beanProductAgent,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(beanProductAgent,QAgent,problem)).
...
% this is what beanProductAgent actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(beanProductAgent,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,beanProductAgent,question,Answer)),
...
% this is to connect beanProductAgent to the server:
...
:- startServer(_),initialiseClient(_), beanProductAgent.
