...
payForAgent :-
	reconsult(payForAgentThy),
	reconsult(payForAgentSig),
	listen.

listen :-
	in(query(QAgent,payForAgent,QueryType,Query)),!,
	write('payForAgent has received a query ..'),nl,
...
% payForAgent, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(payForAgent,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(payForAgent,QAgent,problem)).
...
% this is what payForAgent actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(payForAgent,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,payForAgent,question,Answer)),
...
% this is to connect payForAgent to the server:
...
:- startServer(_),initialiseClient(_), payForAgent.
