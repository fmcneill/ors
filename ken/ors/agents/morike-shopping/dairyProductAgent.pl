...
dairyProductAgent :-
	reconsult(dairyProductAgentThy),
	reconsult(dairyProductAgentSig),
	listen.

listen :-
	in(query(QAgent,dairyProductAgent,QueryType,Query)),!,
	write('dairyProductAgent has received a query ..'),nl,
...
% dairyProductAgent, like all agents, attempts to resolve queries addressed to him


resolve(QueryType,Query,QAgent) :-
	solve(QueryType,Query,Answer,QAgent),
	out(reply(dairyProductAgent,QAgent,Answer)).

resolve(_,_,QAgent) :-
	out(reply(dairyProductAgent,QAgent,problem)).
...
% this is what dairyProductAgent actually does:
...
checkWithAgent([H|T],Agent) :-
	out(query(dairyProductAgent,Agent,question,H)),
	write('have asked about '),write(H),nl,
	in(reply(Agent,dairyProductAgent,question,Answer)),
...
% this is to connect dairyProductAgent to the server:
...
:- startServer(_),initialiseClient(_), dairyProductAgent.
