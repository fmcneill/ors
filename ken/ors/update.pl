...
updateKIFOnt(Assertions,Situation) :-
%	removeDoubles(Assertions,[],NoDupsAssertions),
	reverse(Assertions,RevAssertions),
	translateSituation(Situation,[],TransSituation),
	updateKIFOntRev(RevAssertions,TransSituation).


updateKIFOntRev([],Situation) :-
	updateAllSituations(Situation),
	write('The KIF ontology has been updated'),nl,nl.

updateKIFOntRev([[start]|OtherAssertions],Situation) :-
	updateKIFOntRev(OtherAssertions,Situation).

updateKIFOntRev([FirstState|OtherAssertions],Situation) :-
	FirstState = [_ActionName,FirstAssertionList],
	processAssertions(FirstAssertionList,Situation),
	updateKIFOntRev(OtherAssertions,Situation).


processAssertions([],_).

processAssertions([FirstAssertion|OtherAssertions],Situation) :-
	FirstAssertion =.. [PredName|_],
	name(PredName,PredNameChars),
	name('class',Class),
	matchExpression(_,Class,_,PredNameChars),
	processAssertions(OtherAssertions,Situation).
