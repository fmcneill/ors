...
ors(PA) :-
	...
	consult(ExpClient),
	%BridgeRule
	get_absolute_path('/ORS/bridge_rule/storeBR.pl',StoreBridgeRule),
	consult(StoreBridgeRule),
	get_absolute_path('/ORS/bridge_rule/searchBR.pl',SearchBridgeRule),
	consult(SearchBridgeRule),
	assert(messages([])).
...
%---Check Bridge Rule
%	checkBridge(ListOfSurprisingQuestions,Known,Agent,Action),
	%write('Known Term : '),write(Known),nl,nl,
	...
	%--- Store Bridge Rule-----
	storeBridgerule(store,Action,Diagnosis,ListOfSurprisingQuestions).
