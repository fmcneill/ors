...
diagnoseFailure(Me,OntType,Scenario,Action,Agent,_ListOfQueries,_Just,_SurprisingQues,Experiences,Outcome,experience) :-
	consumeExperiences(Agent,Action,Experiences,Repairs,OntType,Scenario).
...
checkFullPredExists(Pred) :-
	Pred =.. [PredName|Args],
	length(Args,NoArgs),
	setof(X,predicate(X),FullPreds),
	checkEachFullPred(PredName,NoArgs,FullPreds).

checkEachFullPred(RSQ,NoArgs,[FirstPred|Rest]):-
	FirstPred =.. [RSQ|Args],
	length(Args,NoArgs).

checkEachFullPred(RSQ,NoArgs,[_FirstPred|Rest]) :-
	checkEachFullPred(RSQ,NoArgs,Rest).
...
compareArity(Me,Agent,Just,RelevantSurprisingQuestion,_,ArityRSQ,Action,Precond,ArityRSQ,OntType,Scenario,Outcome,Repair) :-
...
propositionalRepair(Scenario,Me,Agent,RSQ,ExPrecond,ArityRSQ,ArityExP,NameRSQ,OntType,Outcome,[ExPrecond,RSQ,[propositionalAA,ExPArgsClassList,[UnmatchedClass,Position]]]) :-
...
propositionalRepair(Scenario,Me,Agent,RSQ,ExPrecond,ArityRSQ,ArityExP,NameRSQ,OntType,Outcome,Repair) :-
	...
	write('My arguments are of these types : '),write(ExPArgsClassList),nl,
	findArgsClassHis(Me,Agent,RSQ,RSQArgsClassList),
	findClassAndPosition(ExPArgsClassList,RSQArgsClassList,UnmatchedClass,Position),
	translatePred(NameRSQ,TransName),
	attemptRefine(Scenario,propositionalA,OntType,[TransName,Position],Outcome),
	%-------Bridge rule modified------------
        Repair = [ExPrecond,RSQ,[propositionalA,ExPArgsClassList,[UnmatchedClass,Position]]].
...
%----Modified some for a bridge rule : include ExPArgsClassList
examineDiffs(Me,Agent,Action,RSQ,ExPrecond,Just,MyDiffList,HisDiffList,ExPArgsClassList,RSQArgsClassList,OntType,Scenario,Outcome,Repair) :-
	MyDiffList = [],
	HisDiffList = [],
	compareClassOrder(RSQArgsClassList,ExPArgsClassList,[],SwitchList),
	checkSwitchList(Me,Agent,Action,RSQ,ExPrecond,Just,SwitchList,OntType,Scenario,Outcome,Repair,ExPArgsClassList).
...
checkSwitchList(Me,Agent,_,RSQ,ExPrecond,_,[],_,_,success,[],ExPArgsClassList) :-
	checkTransitive(Me,Agent,RSQ,ExPrecond),
	write('DIAGNOSIS: Transitive closure.  New Pred is '),write(RSQ),nl.


checkSwitchList(Me,Agent,_,_,ExPrecond,Just,[],OntType,Scenario,Outcome,Repair,ExPArgsClassList) :-
...
%----Modified for a brigde rule :RSQ, ExPArgsClassList, SwitchList
checkSwitchList(_,_,_,RSQ,ExPrecond,_,SwitchList,OntType,Scenario,Outcome,Repair,ExPArgsClassList) :-
...
%---for debug a bridge rule-----------
	 Repair = [ExPrecond,RSQ,[switchArgs,ExPArgsClassList,PredName,SwitchList]],
	write(Repair),nl,
%---------------------------
...
%---Bridge Rule add ExPrecond----
compareDiffs(Me,Agent,_,RSQ,ExPrecond,ExPArgsClassList,_,MyDiff,HisDiff,OntType,Scenario,Outcome,Repair) :-
...
compareDiffs(Me,Agent,ExPrecond,RSQ,_,ExPArgsClassList,_,MyDiff,HisDiff,OntType,Scenario,Outcome,Repair) :-
...
checkPredicateSubTypes(Me,Agent,Action,RSQ,[FirstPrecond|RestPreconds],OntType,Scenario,Outcome,Repair) :-
	...
	checkPredicateSubTypes(Me,Agent,Action,RSQ,[FirstPrecond|RestPreconds],NameRSQ,NameFP,OntType,Scenario,Outcome,Repair).
...
checkPredicateSubTypes(_Me,_Agent,Action,RSQ,[FirstPrecond|_],NameRSQ,NameFP,OntType,Scenario,Outcome,Repair) :-
...
%	Repair = predicate_anti_abstraction,
	...
	write('The appropriate repair has been performed'),nl,
        Repair = [FirstPrecond,RSQ,[predicateAA,[NameRSQ,NameFP]]].
%---Bridge rule modified format Repair------------
checkPredicateSubTypes(_,_,Action,RSQ,[FirstPrecond|_],NameRSQ,NameFP,OntType,Scenario,Outcome,Repair) :-
	predSubclass(NameFP,NameRSQ),
	write('DIAGNOSIS: Predicate abstraction'),nl,
	write(NameFP),write(' is a subtype of '),write(NameRSQ),nl,
	predTranslate(NameFP,NameRSQ,Action,TransInfo),
	attemptRefine(Scenario,predicateAA,OntType,TransInfo,Outcome),
	Repair = [FirstPrecond,RSQ,[predicateA,[NameRSQ,NameFP]]].
...
%---Bridge rule modified format :Repair------------
checkPredicateSubTypes(Me,Agent,Action,RSQ,[FirstPrecond|_],NameRSQ,NameFP,OntType,Scenario,Outcome,Repair) :-
