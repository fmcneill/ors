...
consultMeta(Scenario,RefineType,OntType,RefineInfo,Outcome) :-
	...
	checkProtection(Scenario,RefineType,OntType,RefineInfo,Outcome).
...
% checkProtection(Scenario,precondAA,onto,[Rule,RightClass,WrongClass],[protect,[precondAA,[Rule,RightClass,WrongClass]])
%% checkProtection(Scenario,precondAA,onto,[Rule,RightClass,WrongClass],[protect,[precondAA,[Rule,RightClass,WrongClass]])

checkProtection(Scenario,precondAA,onto,[Rule,RightClass,WrongClass],failure) :-
	protectFunction(FunctionList),
	...
	name(')',RightBracket),
	name(',',Comma),
	name('functionAll',FunctionAll),
	name('highProtection',HighProtection),	
	name('lowProtection',LowProtection),
	% (protect(NewRule,functionAll,_)
	joinFunction(LeftBracket,LowRuleChars,Comma,FunctionAll,Comma,HighProtection,RightBracket,AllHighName),
	joinFunction(LeftBracket,LowRuleChars,Comma,FunctionAll,Comma,LowProtection,RightBracket,AllLowName),
	% protect(NewRule,NewWrongClass,argumentClass,_)
	(
	member(AllHighName,FunctionList),
	write('---Protection used: '),write(AllHighName),nl
	;
	member(AllLowName,FunctionList),
	write('---Protection used: '),write(AllLownName),nl
	),
	repairFailure(Scenario,precondAA,onto,[Rule,RightClass,WrongClass]).
...
% add some argument to the predicate

checkProtection(Scenario,propositionalAA,onto,[Pred,ArgType,ArgValue,Position,Arity],failure) :-
	protectFunction(FunctionList),
	...
	name(',',Comma),
	name('functionAll',FunctionAll),
	name('functionArity',FunctionArity),
	name('highProtection',HighProtection),	
	name('lowProtection',LowProtection),
	% (protect(NewPred,functionAll,_)
	joinFunction(LeftBracket,LowPredChars,Comma,FunctionAll,Comma,HighProtection,RightBracket,AllHighName),
	joinFunction(LeftBracket,LowPredChars,Comma,FunctionAll,Comma,LowProtection,RightBracket,AllLowName),
	% protect(NewPred,functionArity,_)
	joinFunction(LeftBracket,LowPredChars,Comma,FunctionArity,Comma,HighProtection,RightBracket,ArityHighName),
	joinFunction(LeftBracket,LowPredChars,Comma,FunctionArity,Comma,LowProtection,RightBracket,ArityLowName),
	(
	member(AllHighName,FunctionList),
	write('---Protection used: '),write(AllHighName),nl;
	member(AllLowName,FunctionList),
	write('---Protection used: '),write(AllLowName),nl;
	member(ArityHighName,FunctionList),
	write('---Protection used: '),write(ArityHighName),nl;
	member(ArityLowName,FunctionList),
	write('---Protection used: '),write(ArityLowName),nl
	),
	repairFailure(Scenario,propositionalAA,onto,[Pred,ArgType,ArgValue,Position,Arity]).

%% A

% remove some argument 

checkProtection(Scenario,propositionalA,onto,[Pred,ArgPosition],failure) :-
	protectFunction(FunctionList),
	protectArgument(ArgumentList),
	lowerName(Pred,LowPredChars),
	...
	name('highProtection',HighProtection),	
	name('lowProtection',LowProtection),
	% (protect(Pred,functionAll,_)
	joinFunction(LeftBracket,LowPredChars,Comma,FunctionAll,Comma,HighProtection,RightBracket,AllHighName),
	joinFunction(LeftBracket,LowPredChars,Comma,FunctionAll,Comma,LowProtection,RightBracket,AllLowName),
	nl,write(AllHighName),nl,write(AllLowName),
	...
	nl,write(ValueHighName),nl,write(ValueLowName),nl,
	(
	member(AllHighName,FunctionList),
	write('---Protection used: '),write(AllHighName),nl;
	member(AllLowName,FunctionList),
	...
	repairFailure(Scenario,propositionalA,onto,[Pred,ArgPosition]).


% SWITCH ARGUMENTS

checkProtection(Scenario,switchArgs,onto,PredName,failure) :-
	protectFunction(FunctionList),
	protectArgument(ArgumentList),
	...
	joinArgument(LeftBracket,LowPredChars,Comma,Position1,Comma,ArgumentAll,Comma,HighProtection,RightBracket,AllHigh1Name),
	joinArgument(LeftBracket,LowPredChars,Comma,Position1,Comma,ArgumentAll,Comma,LowProtection,RightBracket,AllLow1Name),
	%protect(NewPred,2,argumentAll,_)
	joinArgument(LeftBracket,LowPredChars,Comma,Position2,Comma,ArgumentAll,Comma,HighProtection,RightBracket,AllHigh2Name),
	joinArgument(LeftBracket,LowPredChars,Comma,Position2,Comma,ArgumentAll,Comma,LowProtection,RightBracket,AllLow2Name),
	(
	member(AllHighName,FunctionList),
	write('---Protection used: '),write(AllHighName),nl;
	member(AllLowName,FunctionList),
	write('---Protection used: '),write(AllLowName),nl;
	member(AllHigh1Name,ArgumentList),
	write('---Protection used: '),write(AllHigh1Name),nl;
	member(AllLow1Name,ArgumentList),
	write('---Protection used: '),write(AllLow1Name),nl;
	member(AllHigh2Name,ArgumentList),
	write('---Protection used: '),write(AllHigh2Name),nl;
	member(AllLow2Name,ArgumentList),
	write('---Protection used: '),write(AllLow2Name),nl	
	),
	repairFailure(Scenario,switchArgs,onto,PredName).


% ELSE

checkProtection(Scenario,RefineType,OntType,RefineInfo,success) :-
	repairSuccess(Scenario,RefineType,OntType,RefineInfo).

	
% joinFunction(+El1-El7,-Result)
% joins the elements of the protectFunction predicate

joinFunction(El1,El2,El3,El4,El5,El6,El7,ResName) :-
	append(El1,El2,Res1),
	append(El3,El4,Res2),
...
% repairFailure(+Scenario,+Type,+onto,+Info)
% called when the repair cannot be performed due to protection

repairFailure(Scenario,Type,onto,Info) :-
	write('*** Sorry, the repair cannot be performed'),nl,nl,
	negotiation(Scenario,Type,onto,Info).


% repairSuccess(+Scenario,+Type,+onto,+Info)
% called when the repair can be performed
