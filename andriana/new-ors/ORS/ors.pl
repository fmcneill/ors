...
requestDiagnosis(Plan,Diagnosis,ProtectionInfo,OntType,Scenario,OnOrOff) :-
	...
	checkOutcome(Me,Outcome,Agent,ProtectionInfo),
	shareExps(OnOrOff).


checkOutcome(_Me,success,_Agent,ok).
checkOutcome(_Me,failure,_Agent,ok).
checkOutcome(Me,[protect,Suggestion],Agent,[protect,Outcome,Suggestion]) :-
	out(request(Me,ActionSPA,repair,Suggestion)),
	in_proxy(reply(ActionSPA,Me,repair,Outcome)).
