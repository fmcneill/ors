...
%:- consult('~/trunk/andriana/new-ors/ORS/ors.pl').
%:- consult('~/trunk/andriana/new-ors/ORS/refinement/Ontolingua_refinement/refinement.pl').
start :-
    scenario_name(Sce),
...
plan(OntType,Scenario,OnOrOff,Goal,ActionsSoFar,RefinementsSoFar) :-
	...
	write('translate done'),nl,nl,
	find_ontology_path(Scenario,ScenarioPath),
	write('find ontology done'),nl,nl,
...
%	write('----------to esteila'),nl,nl,
	waitForAnswer(ActionSPA,Outcome).
%write(' the outcome is         :    '),write(Outcome),nl,nl,nl,
%write('ououou'),nl,nl.
...
postActionProcess(OntType,Scenario,OnOrOff,Action,LaterActions,Asserted,Goal,ActionList,RepairList,ActionSPA,_Plan,[protected,ok]) :-
...
postActionProcess(OntType,Scenario,OnOrOff,Action,LaterActions,Asserted,Goal,ActionList,RepairList,ActionSPA,_Plan,[protected,no]) :-
	write('Since we came to an agreement about the repair and the repair was performed, now finally the '),
	write(Action), write(' performed successfully '), nl,
	%processPostconds(Action,ActionSPA,AssertedHere),
	%processPlan(OntType,Scenario,OnOrOff,LaterActions,[[Action,AssertedHere]|Asserted],Goal,[Action|ActionList],RepairList).	
	%updateOnt(OntType,Scenario,Asserted,ActionList),
...
% findDiagnosis(+OntType,+Scenario,+OnOrOff,+Goal,+ActionList,-RepairList,-ActionSPA,+Plan)
% find a diagnosis of a mismatch

findDiagnosis(OntType,Scenario,OnOrOff,Goal,ActionList,RepairList,ActionSPA,Plan) :-
	requestDiagnosis(Plan,ProposedRepair,NegOutcome,OntType,Scenario,OnOrOff,ActionSPA),
	%repairResult(OntType,Scenario,OnOrOff,Goal,ActionList,RepairList,ActionSPA,ProposedRepair,NegOutcome),
	plan(OntType,Scenario,OnOrOff,Goal,ActionList,[ProposedRepair|RepairList]),!.		
	
	
% repairResult(+OntType,+Scenario,+OnOrOff,+Goal,+ActionList,+RepairList,+ActionSPA,+ProposedRepair,+NegOutcome)
% behaves accordingly to the result of negotiation/repair	


repairResult(_OntType,_Scenario,_OnOrOff,_Goal,_ActionList,_RepairList,_ActionSPA,_ProposedRepair,nonePA) :-
	write('* This repair was performed by me.'),nl.
	
repairResult(_OntType,_Scenario,_OnOrOff,_Goal,_ActionList,_RepairList,_ActionSPA,_ProposedRepair,none) :-
	write('* This repair was performed.'),nl.
%	plan(OntType,Scenario,OnOrOff,Goal,ActionList,[ProposedRepair|RepairList]),!.	

repairResult(_OntType,_Scenario,_OnOrOff,_Goal,_ActionList,_RepairList,ActionSPA,_ProposedRepair,[protected,noneSPA]) :-
	write('* This part of my ontology is protected.'),nl,write('* '),
	write(ActionSPA),write(' agreed to perform the repair.'),nl.
	%negativeResponse(ActionSPA,Scenario).
%	plan(OntType,Scenario,OnOrOff,Goal,ActionList,RepairList),!.
	
repairResult(_OntType,_Scenario,_OnOrOff,_Goal,_ActionList,_RepairList,ActionSPA,_ProposedRepair,[protected,doneSPA]) :-
	write('* This part of my ontology is protected.'),nl,write('* '),
	write(ActionSPA),write(' agreed to perform the inverse.'),nl,write('* I asked twice.'),nl.
%	plan(OntType,Scenario,OnOrOff,Goal,ActionList,RepairList),!.	

repairResult(_OntType,_Scenario,_OnOrOff,_Goal,_ActionList,_RepairList,_ActionSPA,_ProposedRepair,[protected,donePA]) :-
	write('* The negotiation agreement allocated to me the repair.'),nl,write('* '),
	%write(ActionSPA),write(' refused to implement the inverse.'),nl,
	write('* Therefore I decided to implement it instead.'),nl.
	%negativeResponse(ActionSPA,Scenario).
%	plan(OntType,Scenario,OnOrOff,Goal,ActionList,[ProposedRepair|RepairList]),!.

repairResult(_OntType,_Scenario,_OnOrOff,_Goal,_ActionList,_RepairList,ActionSPA,_ProposedRepair,[protected,nonePA]) :-
	write('This part of my ontology is protected.  '),
	write(ActionSPA),write(' agreed to implement the inverse.'),nl.
%	plan(OntType,Scenario,OnOrOff,Goal,ActionList,RepairList),!.

repairResult(_OntType,_Scenario,_OnOrOff,_Goal,_ActionList,_RepairList,ActionSPA,_ProposedRepair,[protected,doneSpa]) :-
	write('* This part of my ontology is protected.'),nl,write('* '),
	write(ActionSPA),write(' agreed to perform the inverse.'),nl.
%	plan(OntType,Scenario,OnOrOff,Goal,ActionList,RepairList),!.	
	
repairResult(_OntType,_Scenario,_OnOrOff,_Goal,_ActionList,_RepairList,ActionSPA,_ProposedRepair,[protected,no]) :-
	write('* This part of my ontology is protected.'),nl,write('* '),
	write(ActionSPA),write(' agreed to implement the inverse.'),nl.
	%negativeResponse(ActionSPA,Scenario).
%	plan(OntType,Scenario,OnOrOff,Goal,ActionList,RepairList),!.	

% negativeResponse(+Agent,Scenario)
% mark agent as not able to perform an action

negativeResponse(Agent,Scenario) :-
	write('* '),write(Agent),write(' will not be able to perfom this type of repair for me.'),
	name(Agent,AgentChar),
	convertNameToKIF(AgentChar,AgentKIFChar,[]),
        name(AgentKIF,AgentKIFChar),nl,
        refineMeta(Scenario,agent,AgentKIF).

% convertNameToKIF(+Name,-ConvertedName,+NameSoFar)
% converts a name so that it is readable by prolog

convertNameToKIF([],RevName,NewName) :-
	reverse(RevName,NewName).

convertNameToKIF([FirstChar|Rest],NewName,[]) :-
	FirstChar > 64,
	NewFirstChar is FirstChar - 32,
	convertNameToKIF(Rest,NewName,[NewFirstChar|[]]),!.

convertNameToKIF([Next|Rest],NewName,NewSoFar) :-
	Next > 64,
	Next < 91,
	name('-',[Dash]),
	convertNameToKIF(Rest,NewName,[Next|[Dash|NewSoFar]]),!.

convertNameToKIF([FirstChar|Rest],NewName,NewSoFar) :-
	convertNameToKIF(Rest,NewName,[FirstChar|NewSoFar]),!.
...
waitForAnswer(ActionSPA,Outcome) :-
	write('wait for reply '),nl,
	sleep(1),
	proxyIn(reply(ActionSPA,pa,_Action,Outcome)).

waitForAnswer(ActionSPA,Outcome) :-
	write('wait for query '), nl,	
	sleep(1),
	proxyIn(query(ActionSPA,pa,Type,Ques)),
