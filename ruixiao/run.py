...
def create_run_command(scenario_name, spas, goal, onto_type, experience_sharing): 
  ...
  run_command = '\nkillall sicstus\n\nxterm -geometry "0x0" -e sicstus -Xrs -l $ORS_HOME/agent_environment/server_create.pl &\nsleep 1\n\nxterm -bg "#FFFFFF" -geometry "60x5" -e sicstus --noinfo -l $ORS_HOME/agent_environment/server.pl &\nsleep 1\n\n' + write_spas() + 'sleep 1\n\nxterm -bg "#AFCCF4" -fg "#000000" -geometry "85x27+580+200" -e sicstus --noinfo --nologo -l $ORS_HOME/agent_environment/PA/planningAgent.pl --goal "assert(scenario_name(%s)),assert(goal(%s)),assert(ontoType(%s)), assert(experience_sharing(%s))." &\n\nsleep 2\n\n' % (scenario_name, goal, onto_type, experience_sharing)
...
#create_run_command('shopping', ['buyAgent', 'joinGroupAgentOne', 'joinGroupAgentTwo', 'putInBasketAgent', 'putItemInBasketAgent'], 'has(shoppingAgent,ourMutualFriend)', 'onto', 'off')

create_run_command('sem_matching', ['tomRecruiterAgent'], 'employs(scottishNationalGalleryOfModernArt,jerryTheBot)', 'sumo', 'off')
